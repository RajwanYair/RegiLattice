...
$Replacements = @(
    ...
    @{ File = '.github\copilot-instructions.md'; Old = '~7,718 tweaks, 158 categories, 3,373 tests'; New = "~$TweakFmtK tweaks, $CategoryCount categories, $TestCount tests" }
    ...
    @{ File = '.github\copilot-instructions.md'; Old = '3,373 tests (0 failures)'; New = "$TestCount tests (0 failures)" }
    @{ File = '.github\copilot-instructions.md'; Old = '3,373 passing (0 consistent failures)'; New = "$TestCount passing (0 consistent failures)" }
    # Agent
    @{ File = '.github\agents\regilattice.agent.md'; Old = '7,718 tweaks across 158 categories, 195 modules, 3,373 tests'; New = "$TweakFmtK tweaks across $CategoryCount categories, $ModuleCount modules, $TestCount tests" }
    ...
    @{ File = '.github\instructions\testing.instructions.md'; Old = '| **Total**                | **3,373+**|'; New = "| **Total**                | **$TestCount+**|" }
    @{ File = '.github\instructions\testing.instructions.md'; Old = "all 3,373+ tests must pass"; New = "all $TestCount+ tests must pass" }
)
