...
internal static class SystemRestore
{
    ...
    [
        ...
        new TweakDef
        {
            Id = "restore-set-crash-dump-type-small",
            Label = "Use Small Memory Dump on BSOD",
            Category = "System Restore",
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Configures BSOD crash dumps to store only a small memory dump (256 KB) rather than a full dump.",
            Tags = ["restore", "bsod", "crash", "dump", "disk-space", "debug"],
            RegistryKeys = [$@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl"],
            ApplyOps = [RegOp.SetDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "CrashDumpEnabled", 3)],
            RemoveOps = [RegOp.DeleteValue($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "CrashDumpEnabled")],
            DetectOps = [RegOp.CheckDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "CrashDumpEnabled", 3)],
        },
        new TweakDef
        {
            Id = "restore-allow-dump-overwrite",
            Label = "Allow Overwriting Existing Crash Dump",
            Category = "System Restore",
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Allows Windows to overwrite the previous crash dump file instead of accumulating multiple files.",
            Tags = ["restore", "bsod", "dump", "disk-space", "crash"],
            RegistryKeys = [$@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl"],
            ApplyOps = [RegOp.SetDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "Overwrite", 1)],
            RemoveOps = [RegOp.DeleteValue($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "Overwrite")],
            DetectOps = [RegOp.CheckDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "Overwrite", 1)],
        },
        new TweakDef
        {
            Id = "restore-disable-auto-reboot-after-bsod",
            Label = "Disable Auto-Reboot After BSOD",
            Category = "System Restore",
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Prevents automatic reboot after a Blue Screen of Death so the stop code can be read.",
            Tags = ["restore", "bsod", "reboot", "debug", "crash", "usability"],
            RegistryKeys = [$@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl"],
            ApplyOps = [RegOp.SetDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "AutoReboot", 0)],
            RemoveOps = [RegOp.SetDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "AutoReboot", 1)],
            DetectOps = [RegOp.CheckDword($@"{LmKey}\SYSTEM\CurrentControlSet\Control\CrashControl", "AutoReboot", 0)],
