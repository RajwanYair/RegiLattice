...
internal static class Performance
{
    ...
    [
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Disables background execution of UWP/Store apps via Group Policy. Reduces CPU and memory usage from idle apps. Default: allowed.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Disables NTFS last access timestamp updates. Reduces disk writes and improves file system performance. Uses value 0x80000001 (system managed off). Default: system managed on.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = false,
            CorpSafe = true,
            Description = "Disables thumbnail generation for files on network folders. Prevents slow Explorer loading when browsing network shares. Default: enabled.",
            Tags = ["performance", "thumbnails", "network", "explorer"],
            RegistryKeys = [@"HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced"],
            ApplyOps = [RegOp.SetDword(@"HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "DisableThumbnailsOnNetworkFolders", 1)],
            RemoveOps = [RegOp.DeleteValue(@"HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "DisableThumbnailsOnNetworkFolders")],
            DetectOps = [RegOp.CheckDword(@"HKEY_CURRENT_USER\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "DisableThumbnailsOnNetworkFolders", 1)],
        },
        new TweakDef
        {
            ...
            NeedsAdmin = false,
            CorpSafe = true,
            Description = "Disables window animations (minimize, maximize, open, close). Reduces visual overhead and improves perceived responsiveness. Default: enabled.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Enables hardware-accelerated GPU scheduling. Reduces latency by allowing the GPU to manage its own memory. Requires compatible GPU driver. Default: off.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Sets large page minimum allocation to 128MB. Improves memory performance for applications that support large pages. Default: not set.",
            Tags = ["performance", "memory", "large-pages", "allocation"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management"],
            ApplyOps = [RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management", "LargePageMinimum", 134217728)],
            ...
            DetectOps = [RegOp.CheckDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management", "LargePageMinimum", 134217728)],
        },
        new TweakDef
        {
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = false,
            CorpSafe = true,
            Description = "Sets Windows visual effects to 'Adjust for best performance'. Disables all animations, shadows, thumbnails. Default: Let Windows decide.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = false,
            CorpSafe = true,
            Description = "Reduces the hung application timeout to 1000ms (1 second). Windows detects and prompts to close unresponsive apps faster. Default: 5000ms.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Sets SvcHostSplitThresholdInKB to match installed RAM. Reduces the number of svchost.exe processes on systems with ample memory. Default: auto.",
