// RegiLattice.Core — Tweaks/ActiveDirectory.cs
// Active Directory domain membership hardening tweaks (client-side).
// Slug: "ad" — Netlogon policy, Kerberos settings, DC discovery hardening.
// NOTE: SealSecureChannel, SignSecureChannel, MaximumPasswordAge, MaxClockSkew,
//       CachedLogonsCount, and NTLMMinClientSec are already covered in
//       Hardening.cs / Security.cs / UserAccount.cs — only unique paths are used here.
...
internal static class ActiveDirectory
{
    ...
    [
        ...
        new TweakDef
        {
            ...
            ApplyOps  = [RegOp.SetDword(WinSysPol, "DontEnumerateConnectedUsers", 1)],
            RemoveOps = [RegOp.DeleteValue(WinSysPol, "DontEnumerateConnectedUsers")],
            DetectOps = [RegOp.CheckDword(WinSysPol, "DontEnumerateConnectedUsers", 1)],
        },
    ];
}
