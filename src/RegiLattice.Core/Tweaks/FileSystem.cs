...
internal static class FileSystem
{
    ...
    [
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables the Encrypting File System (EFS) configuration warning dialog. Prevents prompts when EFS is not configured or not in use. Default: 0 (warning enabled). Recommended: disabled on machines not using EFS.",
            ...
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\EFS", "EfsConfiguration", 1),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\EFS", "EfsConfiguration", 0),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Increases the I/O Request Packet (IRP) stack size for the LanmanServer service. Improves reliability and throughput for network file sharing with many concurrent connections. Default: 15. Recommended: 50 for file servers or heavy SMB workloads.",
            ...
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\LanmanServer\Parameters", "IRPStackSize", 50),
            ],
            RemoveOps =
            [
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables the Remote Differential Compression (MSRDC) feature used during remote file sync. Reduces CPU overhead when RDC is unnecessary in local or fast-network environments. Default: 0 (enabled). Recommended: disabled on fast LANs or when RDC is not needed.",
            Tags = ["filesystem", "rdc", "remote", "compression", "sync", "network"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\MSRDC\Parameters"],
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\MSRDC\Parameters", "DisableMSDC", 1),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\MSRDC\Parameters", "DisableMSDC", 0),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Sets the data deduplication service MaxMemory to 2048 MB for faster processing. Allows the dedup engine to use more RAM during optimization passes. Default: not set (engine default). Recommended: 2048 on servers with 16 GB+ RAM.",
            ...
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Dedup\Parameters", "MaxMemory", 2048),
            ],
            RemoveOps =
            [
                RegOp.DeleteValue(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\Dedup\Parameters", "MaxMemory"),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables the Windows Search (WSearch) content indexing service via registry. Eliminates background I/O caused by index building on large volumes. Default: 2 (automatic start). Recommended: disabled (4) on servers or SSD-only machines.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Enables the global flag allowing per-directory NTFS case sensitivity. Required for WSL interop and POSIX-compliant directory behavior on Windows. Default: 1 (case-insensitive). Recommended: 0 (case-sensitive) for WSL/developer use.",
            ...
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "ObCaseInsensitive", 0),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "ObCaseInsensitive", 1),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables automatic 8.3 short name generation for extended (Unicode) characters. Reduces NTFS overhead when creating files with non-ASCII names. Default: 1 (enabled). Recommended: disabled unless legacy 16-bit app compatibility is needed.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Increases the NTFS Master File Table zone reservation from level 1 to level 2. Reserves more contiguous disk space for MFT growth, reducing fragmentation on busy volumes. Default: 1. Recommended: 2 for volumes with many small files.",
            Tags = ["filesystem", "mft", "ntfs", "fragmentation", "reservation"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem"],
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsMftZoneReservation", 2),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsMftZoneReservation", 1),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables NTFS paging file encryption to reduce CPU overhead during memory paging. Improves paging performance at the cost of not encrypting swapped data at rest. Default: 0 (disabled). Recommended: keep disabled unless full-disk encryption is required.",
            Tags = ["filesystem", "paging", "encryption", "swap", "performance", "ntfs"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem"],
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsEncryptPagingFile", 0),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsEncryptPagingFile", 1),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables the Session Manager DOS device mapping protection mode. Allows legacy applications to create global DOS device names without restrictions. Default: 1 (protection enabled). Recommended: disabled only for legacy app compatibility.",
            ...
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager", "ProtectionMode", 0),
            ],
            RemoveOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager", "ProtectionMode", 1),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Sets an additional 1 MB reserved byte margin for critical NTFS disk allocations. Prevents low-disk-space write failures for system-critical operations. Default: not set (0). Recommended: 1048576 (1 MB) on volumes that approach capacity.",
            Tags = ["filesystem", "ntfs", "disk-space", "reserved", "allocation", "margin"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem"],
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsAdditionallyReservedBytes", 1048576),
            ],
            RemoveOps =
            [
                RegOp.DeleteValue(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsAdditionallyReservedBytes"),
            ],
            DetectOps = [RegOp.CheckDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsAdditionallyReservedBytes", 1048576)],
        },
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables Azure AD / Entra ID cloud content indexing via Windows Search policy. Prevents cloud-sourced content from being indexed locally, reducing network and I/O usage. Default: not set (cloud search allowed). Recommended: disabled for privacy-focused setups.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables automatic 8.3 (DOS-compatible) short filename generation on NTFS. Reduces directory enumeration overhead and speeds up file creation. Default: 0 (enabled). Recommended: 1 (disabled).",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Sets NtfsMemoryUsage to 2 (maximum) to allocate more paged pool for NTFS operations. Improves performance on file-heavy workloads. Default: 1. Recommended: 2 on systems with >=16 GB RAM.",
            ...
            RemoveOps =
            [
                RegOp.DeleteValue(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsMemoryUsage"),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Disables NTFS filename tunneling by setting MaximumTunnelEntries to 0. Prevents DOS-era filename compatibility caching. Default: 256. Recommended: 0 on modern systems.",
            Tags = ["filesystem", "ntfs", "tunneling", "performance", "legacy"],
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem"],
            ApplyOps =
            [
                RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "MaximumTunnelEntries", 0),
            ],
            RemoveOps =
            [
                RegOp.DeleteValue(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "MaximumTunnelEntries"),
            ],
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = false,
            Description = "Enables the large system file cache (LargeSystemCache=1). Optimizes memory for file server workloads at the cost of app memory. Default: 0 (desktop). Recommended: 1 for NAS/file server roles.",
        ...
        new TweakDef
        {
            ...
            RegistryKeys = [@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem"],
            ApplyOps = [RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsDisableLastAccessUpdate", unchecked((int)0x80000001))],
            RemoveOps = [RegOp.SetDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsDisableLastAccessUpdate", unchecked((int)0x80000000))],
            DetectOps = [RegOp.CheckDword(@"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem", "NtfsDisableLastAccessUpdate", unchecked((int)0x80000001))],
        },
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Enables a warning when moving encrypted files to an unencrypted location. Prevents accidental decryption. Default: no warning.",
        ...
        new TweakDef
        {
            ...
            NeedsAdmin = true,
            CorpSafe = true,
            Description = "Disables updating the last access time stamp on NTFS files. Reduces write operations and improves file system performance. Default: enabled.",
