    ...
    public static IReadOnlyList<TweakDef> Tweaks =>
        [
            new TweakDef
            {
                ...
                Description = "Sets MaxRequestBytes=16384 in HTTP.sys. Caps the maximum size of the HTTP request entity body buffer accepted by the kernel-mode HTTP stack. Oversized request buffers are a common vector for DoS and buffer-overflow attacks against IIS. 16 KB is the documented default; reducing it further hardens high-security endpoints while having no effect on typical REST APIs that receive JSON payloads under a few kilobytes.",
            ...
            new TweakDef
            {
                ...
                Category    = "IIS Hardening Policy",
                Description = "Sets MaxFieldLength=16384 in HTTP.sys. Restricts the maximum size of any single HTTP request header field. Excessively long header values (e.g., Cookie or Authorization) are exploited in header injection, HTTP request smuggling, and slow-header denial-of-service attacks. Capping individual fields at 16 KB protects the kernel HTTP stack without affecting any legitimate browser or API client.",
            ...
            new TweakDef
            {
                ...
                Category    = "IIS Hardening Policy",
                Description = "Sets DisableSocketPool=1 in W3SVC Parameters. Disables IIS socket pooling, which pre-allocates a pool of listening sockets shared across all web sites bound to the same IP address. Socket pooling can allow one site's TLS configuration to influence another site on the same IP. Disabling it gives each site an isolated socket lifecycle and prevents cross-site socket interference in multi-tenant IIS deployments.",
            ...
            new TweakDef
            {
                ...
                Category    = "IIS Hardening Policy",
                Description = "Sets MaxConnections=10000 in W3SVC Parameters. Caps the total number of simultaneous TCP connections IIS will accept across all sites. Without an explicit cap IIS uses the OS default which is effectively unlimited, leaving the server vulnerable to connection-flood attacks. 10 000 connections accommodates legitimate enterprise HTTP/1.1 and HTTP/2 traffic while preventing unbounded memory and thread consumption.",
