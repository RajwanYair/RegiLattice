...
public static partial class PipManager
{
    ...
    public static async Task<List<string>> ListInstalledAsync(string pythonExe = "python", bool userOnly = false, CancellationToken ct = default)
    {
        ...
        return packages.Select(p => p.Name).Where(n => !string.IsNullOrEmpty(n)).OrderBy(n => n, StringComparer.OrdinalIgnoreCase).ToList();
    }

    /// <summary>Install a pip package.</summary>
    public static async Task InstallAsync(string name, string pythonExe = "python", bool userOnly = true, CancellationToken ct = default)
    {
        ValidateName(name);
    ...
    public static async Task UninstallAsync(string name, string pythonExe = "python", CancellationToken ct = default)
    {
        ValidateName(name);
        var (code, _, stderr) = await ShellRunner.RunAsync(pythonExe, ["-m", "pip", "uninstall", "-y", name], ct).ConfigureAwait(false);
        if (code != 0)
            throw new InvalidOperationException($"pip uninstall failed: {stderr.Trim()}");
    }

    /// <summary>Upgrade a pip package.</summary>
    public static async Task UpgradeAsync(string name, string pythonExe = "python", bool userOnly = true, CancellationToken ct = default)
    {
        ValidateName(name);
