...
internal sealed class BrightnessSchedulerDialog : BaseDialog
{
    ...
    private readonly TrackBar   _trkDay        = new() { Minimum = 0, Maximum = 100, Value = 80, TickFrequency = 10, Width = 340 };
    ...
    private readonly DateTimePicker _dtpDayTime = new() { Format = DateTimePickerFormat.Time, ShowUpDown = true, Width = 100 };

    private readonly Label      _lblNight      = new() { Text = "Night brightness (%)", AutoSize = true };
    private readonly TrackBar   _trkNight      = new() { Minimum = 0, Maximum = 100, Value = 40, TickFrequency = 10, Width = 340 };
    ...
    private readonly DateTimePicker _dtpNightTime = new() { Format = DateTimePickerFormat.Time, ShowUpDown = true, Width = 100 };

    private readonly Button     _btnApplyNow   = new() { Text = "Apply Now",     Width = 100, Height = 28 };
    private readonly Button     _btnSave       = new() { Text = "Save Settings", Width = 110, Height = 28 };
    private readonly Button     _btnClose      = new() { Text = "Close",         Width = 75,  Height = 28, DialogResult = DialogResult.Cancel };
    private readonly Label      _lblStatus     = new() { Dock = DockStyle.Bottom, Height = 26, TextAlign = ContentAlignment.MiddleLeft, Padding = new Padding(6, 0, 0, 0) };
    ...
    private readonly Label      _lblNote       = new()
    {
        ...
        AutoSize = true, ForeColor = Color.FromArgb(0, 100, 160),
    };
    ...
    private void BuildLayout()
    {
        ...
        int row = 0;
        pnl.Controls.Add(_chkEnabled,     0, row); pnl.SetColumnSpan(_chkEnabled, 3); row++;
        pnl.Controls.Add(new Label { Height = 8, Dock = DockStyle.Fill }, 0, row); pnl.SetColumnSpan(pnl.Controls[^1], 3); row++;

        pnl.Controls.Add(_lblDay,         0, row);
        pnl.Controls.Add(_trkDay,         1, row);
        pnl.Controls.Add(_lblDayVal,      2, row); row++;

        pnl.Controls.Add(_lblDayTime,     0, row);
        pnl.Controls.Add(_dtpDayTime,     1, row); row++;

        pnl.Controls.Add(new Label { Height = 8, Dock = DockStyle.Fill }, 0, row); pnl.SetColumnSpan(pnl.Controls[^1], 3); row++;

        pnl.Controls.Add(_lblNight,       0, row);
        pnl.Controls.Add(_trkNight,       1, row);
        pnl.Controls.Add(_lblNightVal,    2, row); row++;

        pnl.Controls.Add(_lblNightTime,   0, row);
        pnl.Controls.Add(_dtpNightTime,   1, row); row++;

        pnl.Controls.Add(_lblNote,        0, row); pnl.SetColumnSpan(_lblNote, 3); row++;
    ...
    private static bool SetBrightness(int pct)
    {
        ...
        catch { /* WMI channel not available */ }

        // Fallback: powercfg active scheme brightness
        try
        {
            using var proc = new System.Diagnostics.Process();
            proc.StartInfo = new System.Diagnostics.ProcessStartInfo("powercfg",
                $"/SETACVALUEINDEX SCHEME_CURRENT SUB_VIDEO VIDEOADAPT {pct}")
            {
                CreateNoWindow = true,
