...
internal sealed class WakeOnLanDialog : BaseDialog
{
    ...
    private readonly Button _btnEnable  = new() { Text = "Enable WoL",  Width = 105, Height = 28, Enabled = false };
    private readonly Button _btnDisable = new() { Text = "Disable WoL", Width = 105, Height = 28, Enabled = false };
    ...
    private readonly Label  _lblNote    = new()
    {
        ...
        ForeColor = Color.FromArgb(0, 100, 160), Padding = new Padding(6, 0, 0, 0),
    };
    ...
    private static List<AdapterInfo> DiscoverAdapters()
    {
        ...
        var macMap = NetworkInterface.GetAllNetworkInterfaces()
    ...
    private void Populate()
    {
        ...
        foreach (AdapterInfo a in _adapters)
        {
            ...
            if (!a.WolEnabled) lvi.ForeColor = Color.FromArgb(128, 128, 128);
            _list.Items.Add(lvi);
        }
        _list.EndUpdate();
    }
    ...
    private async Task SetWolAsync(bool enable)
    {
        ...
        _lblStatus.Text = success
            ? $"✓ WoL {(enable ? "enabled" : "disabled")} for {adapter.Name}."
