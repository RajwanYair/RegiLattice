...
internal sealed class DashboardPanel : Panel
{
    ...
    // ── Public API ─────────────────────────────────────────────────────────
    internal void SetData(TweakEngine engine, IReadOnlyDictionary<string, TweakResult> statusCache, IEnumerable<string> recentTweaks)
    {
        _engine = engine;
        ...
        int skipped = statusCache.Values.Count(r => r is TweakResult.SkippedCorp or TweakResult.SkippedBuild or TweakResult.SkippedHw);
    ...
    private static List<CategoryStat> BuildCategoryStats(TweakEngine engine, IReadOnlyDictionary<string, TweakResult> cache)
    {
        ...
        return counts.Select(kv => new CategoryStat(kv.Key, kv.Value.total, kv.Value.applied)).OrderByDescending(s => s.Applied).Take(8).ToList();
    }
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        DrawStatCard(g, new Rectangle(pad, cardY, cardW, cardH), _score.Label, "Health Score", _score.Color);
        DrawStatCard(g, new Rectangle(pad + (cardW + cardGap), cardY, cardW, cardH), _score.Applied.ToString(), "Applied", AppTheme.Green);
        DrawStatCard(g, new Rectangle(pad + (cardW + cardGap) * 2, cardY, cardW, cardH), _score.NotApplied.ToString(), "Not Applied", AppTheme.FgDim);
        DrawStatCard(g, new Rectangle(pad + (cardW + cardGap) * 3, cardY, cardW, cardH), _score.Total.ToString(), "Total Tweaks", AppTheme.Accent);
    ...
    private void DrawCategoryBars(Graphics g, Rectangle r)
    {
        ...
        for (int i = 0; i < _catStats.Count; i++)
        {
            ...
            g.DrawString($"{stat.Applied}/{stat.Total}", catFont, countBrush, r.X + labelW + maxBarW + 6, midY);
        }
    }
    ...
    private void RepositionButtons(int x = 24, int y = 260)
    {
        if (InvokeRequired)
        {
            BeginInvoke(() => RepositionButtons(x, y));
            return;
        }
