...
internal sealed class PackagesHubPanel : Panel
{
    ...
    internal PackagesHubPanel()
    {
        ...
        _tabs.TabPages.AddRange(new TabPage[] { _tabScoop, _tabWinGet, _tabChoco, _tabPip, _tabPS, _tabMarketplace });
    ...
    private void OnTabSelected(object? sender, EventArgs e)
    {
        ...
        {
            case 0:
                EnsureEmbedded(ref _scoopEmbed, _tabScoop, CreateScoopEmbed);
                break;
            case 1:
                EnsureEmbedded(ref _wingetEmbed, _tabWinGet, CreateWinGetEmbed);
                break;
            case 2:
                EnsureEmbedded(ref _chocoEmbed, _tabChoco, CreateChocoEmbed);
                break;
            case 3:
                EnsureEmbedded(ref _pipEmbed, _tabPip, CreatePipEmbed);
                break;
            case 4:
                EnsureEmbedded(ref _psEmbed, _tabPS, CreatePSEmbed);
                break;
            case 5:
                EnsureEmbedded(ref _marketplaceEmbed, _tabMarketplace, CreateMarketplaceEmbed);
                break;
        }
    }
    ...
    private static Panel CreateScoopEmbed() => BuildEmbeddedPanel(new ScoopManagerDialog());

    private static Panel CreateWinGetEmbed() => BuildEmbeddedPanel(new WinGetManagerDialog());

    private static Panel CreateChocoEmbed() => BuildEmbeddedPanel(new ChocolateyManagerDialog());

    private static Panel CreatePipEmbed() => BuildEmbeddedPanel(new PipManagerDialog());

    private static Panel CreatePSEmbed() => BuildEmbeddedPanel(new PSModuleManagerDialog());

    private static Panel CreateMarketplaceEmbed() => BuildEmbeddedPanel(new MarketplaceDialog());
    ...
    {
        var container = new Panel { BackColor = AppTheme.Bg };
